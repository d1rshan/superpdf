...
**LLM providers**: All LLM work (extraction, pairwise comparison) uses the opencode Go gateway with `muse-spark-contributor` as the default model, configured via environment variables and called with non-streaming structured-output requests (the gateway's muse streaming is unreliable). Any model must be swappable by env change. Embeddings use OpenAI `text-embedding-3-small` (this provider is used for embeddings only).
